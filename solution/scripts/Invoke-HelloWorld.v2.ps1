...
.DESCRIPTION
    Invoke-HelloWorld.v2 is a demonstration script that displays PowerShell invocation information,
...
.PARAMETER Force64Bit
    Forces the script to relaunch in 64-bit PowerShell if executes in a 32-bit process.

.EXAMPLE
    .\Invoke-HelloWorld.v2.ps1

    Runs the script with default parameters.

.EXAMPLE
    .\Invoke-HelloWorld.v2.ps1 -Verbose

    Runs the script with verbose output showing invocation details.

.EXAMPLE
    .\Invoke-HelloWorld.v2.ps1 -TestVariable "myValue" -Force64Bit
