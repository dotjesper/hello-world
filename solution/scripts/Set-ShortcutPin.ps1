...
process {
    try {
        ...
        # WORKAROUND: Shell.Application COM object is required for shell verb discovery and invocation - no PowerShell cmdlet equivalent exists
        ...
        if ($PSCmdlet.ParameterSetName -eq "Shortcut") {
            # WORKAROUND: WScript.Shell COM object is required to read shortcut properties - no PowerShell cmdlet equivalent exists
